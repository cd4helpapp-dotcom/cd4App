...
const normalizeHistory = (value: unknown): TriageHistoryItem[] => {
  ...
  // Keep the complete recent consultation transcript for the doctor PDF.
  // The PDF renderer paginates it instead of silently dropping older answers.
  return finalHistory.slice(-200);
};
...
const ensureAppointmentTriageReport = async (args: {
  ...
  const qaSnapshotLines = buildVoiceChatQaSnapshotLines(history, concern, 50);
  const aiChatTimelineLines = buildAiChatTimelineLines(history, 200);
